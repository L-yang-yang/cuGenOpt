...
SolveResult<typename Problem::Sol> solve(Problem& prob, const SolverConfig& cfg,
    ...
    if (total_smem > (size_t)prop.sharedMemPerBlock) {
        smem_overflow = (prob_smem > 0);  // 记录是否因溢出丢弃了 problem data
        prob_smem = 0;
    ...
    if (auto_pop) {
        ...
        if (prob_smem > 0) {
            ...
            //   所有 block 共享同一份只读数据（如距离矩阵），L2/ws 反映
            //   cache 能容纳多少份工作集的随机访问热子集
            //   下限 = pow2_ceil(SM数)，确保每个 SM 至少 1 个 block
            //
            // T4 实测验证（ch150, L2=4MB, ws=90KB）：
            //   L2/ws=47 → pow2_floor=32 → max(32, pow2_ceil(40)=64) = 64 ✓
            ...
            // 下限：pow2_ceil(SM数)，确保每个 SM 至少 1 block
            int sm_min = 1;
            while (sm_min < prop.multiProcessorCount) sm_min *= 2;
            if (pop_size < sm_min) pop_size = sm_min;
        }
    ...
    while (gen_done < cfg.max_gen) {
        ...
        if (use_aos) {
            ...
            // --- 第二层：算子权重更新（EMA） ---
            {
                ...
                float floor_val = cfg.aos_weight_floor / seq_reg.count;
                if (floor_val < uniform * 0.5f) floor_val = uniform * 0.5f;
                float global_cap = cfg.aos_weight_cap;
