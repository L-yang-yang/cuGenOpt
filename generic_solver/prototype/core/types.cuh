...
constexpr int MAX_SEQ = 24;   // 最大序列数（Perm 全算子+LNS ~16，Binary ~10，留余量）
...
// 构建序列注册表（CPU 端，solve() 初始化时调用）
inline SeqRegistry build_seq_registry(EncodingType encoding, int dim1,
                                       float cross_row_prob = 0.0f,
                                       RowMode row_mode = RowMode::Single) {
    SeqRegistry reg;
    reg.count = 0;
    for (int i = 0; i < MAX_SEQ; i++) { reg.ids[i] = -1; reg.weights[i] = 0.0f; reg.max_w[i] = 0.0f; }
    
    auto add = [&](int id, float w, float cap = 0.0f) {
    ...
    if (encoding == EncodingType::Permutation) {
        // 行内元素级（始终可用）
        add(seq::SEQ_PERM_SWAP,    1.0f);
        add(seq::SEQ_PERM_REVERSE, 1.0f);
        add(seq::SEQ_PERM_INSERT,  1.0f);
        add(seq::SEQ_PERM_3OPT,    0.5f);  // 稍低权重（计算量大）
        
        // 行内片段级
        add(seq::SEQ_PERM_OR_OPT,  0.8f);
        
        // 跨行操作（dim1 > 1 且 cross_row_prob > 0）
        if (dim1 > 1 && cross_row_prob > 0.0f) {
            float cr = cross_row_prob;  // 用 cross_row_prob 缩放跨行权重
            add(seq::SEQ_PERM_CROSS_RELOCATE, 0.6f * cr);
            add(seq::SEQ_PERM_CROSS_SWAP,     0.6f * cr);
            add(seq::SEQ_PERM_SEG_RELOCATE,   0.5f * cr);
            add(seq::SEQ_PERM_SEG_SWAP,       0.5f * cr);
            add(seq::SEQ_PERM_CROSS_EXCHANGE,  0.4f * cr);
        }
        
        // 行级（dim1 > 1）
        if (dim1 > 1) {
            add(seq::SEQ_ROW_SWAP,    0.3f);
            add(seq::SEQ_ROW_REVERSE, 0.2f);
            if (row_mode == RowMode::Partition) {
                add(seq::SEQ_ROW_SPLIT,   0.2f);
                add(seq::SEQ_ROW_MERGE,   0.2f);
            }
        }
        // LNS（Permutation 行内，计算量大，per-op cap=0.01 防止 AOS 过度提权）
        add(seq::SEQ_LNS_SEGMENT_SHUFFLE, 0.006f, 0.01f);
        add(seq::SEQ_LNS_SCATTER_SHUFFLE, 0.006f, 0.01f);
        add(seq::SEQ_LNS_GUIDED_REBUILD,  0.006f, 0.01f);
    }
    else if (encoding == EncodingType::Binary) {
        // 行内元素级
        add(seq::SEQ_BIN_FLIP, 1.0f);
        add(seq::SEQ_BIN_SWAP, 0.8f);
        
        // 行内片段级
        add(seq::SEQ_BIN_SEG_FLIP, 0.6f);
        add(seq::SEQ_BIN_K_FLIP,   0.6f);
        
        // 跨行
        if (dim1 > 1 && cross_row_prob > 0.0f) {
            float cr = cross_row_prob;
            add(seq::SEQ_BIN_CROSS_SWAP,     0.5f * cr);
            add(seq::SEQ_BIN_SEG_CROSS_SWAP, 0.4f * cr);
        }
    ...
    else if (encoding == EncodingType::Integer) {
        // 行内元素级
        add(seq::SEQ_INT_RANDOM_RESET, 1.0f);
        add(seq::SEQ_INT_DELTA,        1.0f);
        add(seq::SEQ_INT_SWAP,         0.8f);
        
        // 行内片段级
        add(seq::SEQ_INT_SEG_RESET,    0.6f);
        add(seq::SEQ_INT_K_DELTA,      0.6f);
        
        // 跨行
        if (dim1 > 1 && cross_row_prob > 0.0f) {
            float cr = cross_row_prob;
            add(seq::SEQ_INT_CROSS_SWAP, 0.5f * cr);
        }
